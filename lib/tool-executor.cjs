...
async function assertPublicAllowedRepo(repo, env, fetchImpl) {
...
async function executeTool({ tool, action, input = {}, env = process.env, fetchImpl = fetch, approved = false }) {
  ...
  if (action === "create_branch") {
    ...
    const base = await githubRequest({ path: "/repos/" + repo + "/git/ref/heads/" + encodeURIComponent(baseRef), env, fetchImpl });
  ...
  if (action === "create_or_update_file") {
    ...
    let currentSha = null;
    try {
      const current = await githubRequest({ path: "/repos/" + repo + "/contents/" + path.split("/").map(encodeURIComponent).join("/") + "?ref=" + encodeURIComponent(branch), env, fetchImpl });
