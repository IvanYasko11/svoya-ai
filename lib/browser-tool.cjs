...
function verifyBrowserResult({ action, finalUrl, title = "", text = "", allowedHosts }) {
  const reasons = [];
  if (!finalUrl) reasons.push("Missing final URL.");
  else if (!isAllowedHost(new URL(finalUrl).hostname, allowedHosts)) reasons.push("Final URL is not allowlisted.");
  if (action === "extract_text" && !String(text).trim()) reasons.push("Expected extracted text is empty.");
  if (String(text).length > MAX_TEXT) reasons.push("Extracted text exceeds the configured limit.");
  if (action === "open" && !String(title).trim() && !String(finalUrl).trim()) reasons.push("Open result has neither title nor URL.");
  return {
    status: reasons.length ? "FAIL" : "PASS",
    reason: reasons.join(" ") || "Deterministic browser result checks passed."
  };
}

module.exports = { executeBrowserTool, normalizeUrl, normalizeAction, isAllowedHost, isPrivateIp, validateBrowserTarget, verifyBrowserResult, ALLOWED_ACTIONS };
